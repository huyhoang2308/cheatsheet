...
```python
df[['A','B']].groupby('C').mean()
```
![groupby_mean](images/md/groupby_mean().png)

-  group the rows based on the values in the column and aggregate
```python
df[['Gender','Exited']].groupby('Gender').agg(['mean','count'])
```
![groupby_agg](images/md/groupby_agg().png)


- Group by multiple columns by passing a list of columns to groupby function.
```python
df[['A','B','C']].groupby(['A','B']).mean()
```

- Group by multiple columns by passing a list of columns to groupby function and sort by condition
```python
df[['A','B','C']].groupby(['A','B']).mean().sort_values(by='C')
```

- Drop empty column
```python
df[['A','B']].groupby('A', dropna=True).agg(['mean','count'])
```
## Idioms
```python
...
# option: pass rows explicitly to skip rows
...
# option: read column names and then data
